...
public class PanelSelectPhongLeTan extends JFrame {
    ...
    public PanelSelectPhongLeTan(ThuePhongDTO x, ArrayList<PhongDTO> lp, DatPhongLeTan dp, int countday, int counthour, JTable tbPhong) {
        initComponents(x, lp, dp, countday, counthour, tbPhong);
    }

    public void initComponents(ThuePhongDTO tp, ArrayList<PhongDTO> lp, DatPhongLeTan dp, int countday, int counthour, JTable tbPhong) {
        for (PhongDTO x : PhongBUS.getListPhong(tp.getNgayThue(), tp.getNgayTra())) {
            listPhong.add(x);
        }
        setVisible(true);
        setSize(500, 500);
    ...
    public void renderTB() {
        ...
        for (TienIchDTO x : listTI) {
            Object row[] = {i, x.getTenTienIch(), listInt.get(i)};
            dtm.addRow(row);
            i++;
        }
